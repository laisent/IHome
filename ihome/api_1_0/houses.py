...
def save_house_info():
    ...
    # 判断金额是否正确
    try:
        price = (float(price) * 100)  # 数据库设计 字段定位：分
    ...
    if facilities_ids:
        try:
            # ["1", "2"]  select * from ih_facility_info where id in [1, 2]
...
def save_house_image():
    ...
    iamge_data = image_file.read()
    # 保存图片到七牛中
    try:
        file_name = storage(iamge_data)
    except Exception as e:
        current_app.logger.error(e)
...
# GET  /api/v1.0/houses?sd=202-02-18&ed=2020-02-23&aid=10&sk=new&p=1
@api.route('/houses')
def get_house_list():
    """"获取房屋的列表信息(搜索页面)"""
    ...
    try:
        if start_date:
            # 字符串 -> 时间 strptime  时间 -> 字符串 strftime
    ...
    try:
        resp_json = redis_store.hget(redis_key, page).decode("utf-8")
    except Exception as e:
        current_app.logger.error(e)
    else:
        if resp_json:
    ...
    elif sort_key == "price-inc":  # 价格从低 -> 高
        house_query = House.query.filter(*filter_params).order_by(House.price.asc())
    elif sort_key == "price-des":  # 价格从低 -> 高
        house_query = House.query.filter(*filter_params).order_by(House.price.desc())
    elif sort_key == "new":  # 新旧
    ...
    resp_dict = dict(errno=RET.OK, eerrmsg="OK", data={"total_page": total_page, "houses": houses, "current_page": page})
    resp_json = json.dumps(resp_dict)
