...
def set_user_avatar():
    ...
    """
    # 装饰器的代码已经将user_id保存到g对象中 所以视图中可以直接读取
    user_id = g.user_id
...
def get_user_profile():
    ...
    if user is None:
        return jsonify(errno=RET.NODATA0, errmsg="无效操作")
...
def get_user_auth():
    ...
    try:
        ...
        current_app.logger.error(e)
        return jsonify(errno=RET.DBERR, errmsg="获取用户实名制信息失败")
...
def set_user_auth():
    ...
    try:
        ...
        current_app.logger.error(e)
        db.session.rollback()
        return jsonify(errno=RET.DBERR, errmsg="保存用户实名制信息失败")
